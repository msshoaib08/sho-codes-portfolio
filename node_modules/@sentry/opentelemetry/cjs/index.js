...
function getDynamicSamplingContextFromSpan(span) {
  ...
  const dscOnSpan = traceStateDsc ? utils.baggageHeaderToDynamicSamplingContext(traceStateDsc) : undefined;

  if (dscOnSpan) {
    return dscOnSpan;
  }

  const dsc = core.getDynamicSamplingContextFromClient(span.spanContext().traceId, client);

  const attributes = spanHasAttributes(span) ? span.attributes : {};

  const sampleRate = attributes[core.SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE];
  if (sampleRate != null) {
...
function getPropagationContextFromSpan(span) {
  ...
  const dsc = traceStateDsc || getDynamicSamplingContextFromSpan(core.getRootSpan(span));
...
function makeTraceState({
  ...
  const traceStateBase = parentSpanId
    ? new core$1.TraceState().set(SENTRY_TRACE_STATE_PARENT_SPAN_ID, parentSpanId)
    : new core$1.TraceState();
...
function getInjectionData(context)

 {
  ...
  const scope = _optionalChain$4([getScopesFromContext, 'call', _8 => _8(context), 'optionalAccess', _9 => _9.scope]) || core.getCurrentScope();
...
function getCurrentURL(span) {
  const urlAttribute = _optionalChain$4([core.spanToJSON, 'call', _11 => _11(span), 'access', _12 => _12.data, 'optionalAccess', _13 => _13[semanticConventions.SEMATTRS_HTTP_URL]]);
  if (urlAttribute) {
    return urlAttribute;
  }
  ...
  class SentryContextManager extends ContextManagerClass {
     ...
     */
     with(
      ...
      const currentScope = _optionalChain$2([currentScopes, 'optionalAccess', _ => _.scope]) || core.getCurrentScope();
      const currentIsolationScope = _optionalChain$2([currentScopes, 'optionalAccess', _2 => _2.isolationScope]) || core.getIsolationScope();
